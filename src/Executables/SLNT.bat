...
copy /y "OldNewExplorer64.dll" "%ALLUSERSPROFILE%\OldNewExplorer"
icacls "%ALLUSERSPROFILE%\OldNewExplorer" /reset /t

REG ADD "HKLM\SOFTWARE\Classes\CLSID\{27DD0F8B-3E0E-4ADC-A78A-66047E71ADC5}\InprocServer32" /ve /t REG_SZ /d "%ALLUSERSPROFILE%\OldNewExplorer\OldNewExplorer64.dll" /f
...
for /f "usebackq tokens=2 delims=\" %%A in (`reg query "HKEY_USERS" ^| findstr /r /x /c:"HKEY_USERS\\S-.*" /c:"HKEY_USERS\\AME_UserHive_[^_]*"`) do (
    ...
    reg query "HKU\%%A" | findstr /c:"Volatile Environment" /c:"AME_UserHive_" > NUL 2>&1
        if not errorlevel 1 call :REG "%%A"
)
